...
`u8 buf[2];`定義兩個字節的8位元變數
`u8 t=0,a=0;`
`I2C_READ_BUFFER(LM75A_ADD,0x00,buf,2);`讀出溫度數據
`t = buf[0];`取出溫度數據中的整數位
`*Tempbuffer = 0;`將溫度正負值設為默認(正)
```
...
```
`if(t & 0x01){ a=a+1; }`如果第一位為1就累加對應數值，其他以此類推
`if(t & 0x02){ a=a+2; }`
`if(t & 0x04){ a=a+4; }`
`if(t & 0x08){ a=a+8; }`
`if(t & 0x10){ a=a+16; }`
`if(t & 0x20){ a=a+32; }`
`if(t & 0x40){ a=a+64; }`
`Tempbuffer++;`將儲存數據的變數改為下一項
`*Tempbuffer = a;`輸入資料
`a = 0;`
`t = buf[1];` //取出小數部分，取0.125精度的前2位（12、25、37、50、62、75、87）
`if(t & 0x20){ a=a+12; }`如果第六位為1就累加對應數值，其他以此類推
`if(t & 0x40){ a=a+25; }`
`if(t & 0x80){ a=a+50; }`
`Tempbuffer++;`將儲存數據的變數改為下一項
`*Tempbuffer = a;`輸入資料
